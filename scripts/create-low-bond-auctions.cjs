...
async function main() {
  ...
  console.log("Creating low-bond test auctions on contract:", contractAddress);

  const OrionMomentumSeries = await hre.ethers.getContractAt("OrionMomentumSeries", contractAddress);

  // Test auction data with 0.01 ETH bond for all
  const auctions = [
    {
      auctionId: "cyber-low-001",
      ...
      bidBond: hre.ethers.parseEther("0.01"),
    },
    {
      auctionId: "digital-low-002",
      metadataURI: JSON.stringify({
    ...
    {
      auctionId: "abstract-low-005",
      metadataURI: JSON.stringify({
        title: "Abstract Reality",
        description: "Mind-bending abstract composition challenging perception of reality",
        image: "https://images.unsplash.com/photo-1557672172-298e090bd0f1?w=800",
      }),
      minBid: hre.ethers.parseEther("0.15"),
      buyNowPrice: hre.ethers.parseEther("1.5"),
      duration: 1 * 24 * 60 * 60, // 1 day
      bidBond: hre.ethers.parseEther("0.01"),
    },
  ];

  console.log(`\nCreating ${auctions.length} test auctions with 0.01 ETH bond...\n`);
  ...
  for (const auction of auctions) {
    try {
      ...
      await tx.wait();
      console.log(`  ✅ Auction created successfully (Bond: 0.01 ETH)\n`);
    } catch (error) {
      console.error(`  ❌ Failed to create auction ${auction.auctionId}:`, error.message);
    }
  }

  console.log("\n✅ All low-bond test auctions created!");
